...
vec3 trace(vec3 from, vec3 dir, inout vec3 hit, inout vec3 hitNormal) {
	...
	if ( dist < epsModified) {
		...
		if (floorHit) {
			...
			hitNormal= normal(hit-0.01*direction, epsModified); // /*normalE*epsModified/eps*/
		}
...
vec3 trace2(vec3 from, vec3 dir, inout vec3 hit, inout vec3 hitNormal)
{
   float frensel = Reflection*2.0;
   vec3 col = trace(from, dir, hit, hitNormal);
   if (hitNormal == vec3(0.0))
     return col;

   vec3 d = reflect(dir, hitNormal);
   float ref = pow(dot(dir, hitNormal) + 1.0, frensel);
   return mix(col, backgroundColor(d), ref);
}

vec3 traceRefract(vec3 from, vec3 dir, inout vec3 hit, inout vec3 hitNormal, int level)
{
    vec3 col = trace(from, dir, hit, hitNormal);      
    if (hitNormal == vec3(0.0))
      return col;
    ...
    {
      vec3 hit2 = hit;
      vec3 hitNormal2 = hitNormal;
      col = trace(hit - hitNormal*mind, dir, hit2, hitNormal2);
      if (hitNormal2 != vec3(0.0))
      {
        {
          vec3 hit3 = hit;
          vec3 hitNormal3 = hitNormal2;
          col = trace(hit - hitNormal2*mind, dir, hit3, hitNormal3);
          if (hitNormal3 != vec3(0.0))
          {
             vec3 d = reflect(dir, hitNormal3);
             float ref = pow(dot(dir, hitNormal3) + 1.0, frensel);
             col = mix(col, trace2(hit3+d*mind,d,hit3, hitNormal3), ref);
          }
         }
         vec3 d = reflect(dir, hitNormal2);
         float ref = pow(dot(dir, hitNormal2) + 1.0, frensel);
         col = mix(col, trace2(hit2+d*mind,d,hit2, hitNormal2), ref);
      }
    }
 //   return col;
    float ref = pow(dot(dir, hitNormal) + 1.0, frensel);
    vec3 d = reflect(dir, hitNormal);
    return mix(col, trace2(hit+d*mind,d,hit, hitNormal), ref);
}
...
vec3 color(vec3 from, vec3 dir) {
	...
	if (Reflection==0.) {
     ...
     {
          return traceRefract(from, dir, hit, hitNormal, 0);
