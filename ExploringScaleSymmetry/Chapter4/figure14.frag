#info Bubbles Distance Estimator.
#define providesInit
...
float DE(vec3 p)
{
     ...
     // find distance to nearest face of a radial tetrahedron
     float l0 = dot(p, v0);
     float l1 = dot(p, v1);
     float l2 = dot(p, v2);
     float l3 = dot(p, v3);
     vec3 dir1, dir2;
     if (min(l0, l1) > max(l2, l3))
     { 
        dir1 = v0; 
        dir2 = v1; 
     }
     else if (min(l0, l2) > max(l1, l3))
     { dir1 = v0; dir2 = v2; }
     else if (min(l0, l3) > max(l1, l2))
     { dir1 = v0; dir2 = v3; }
     else if (min(l1, l2) > max(l0, l3))
     { dir1 = v1; dir2 = v2; }
     else if (min(l1, l3) > max(l0, l2))
     { dir1 = v1; dir2 = v3; }
     else 
     { dir1 = v2; dir2 = v3; }
     vec3 norm = cross(dir1, dir2);
     normalize(norm);
     
     float d = (abs(dot(p, norm)) / dScale) - 0.001;
